...
describe('StorageWidget (Agent)', () => {
  ...
  it('fetches and displays storage used space and free space in B, KB, MB, GB format', async () => {
    ...
    // Switch unit to MB
    const mbButton = screen.getByRole('button', { name: /mb/i });
    fireEvent.click(mbButton);

    expect(screen.getByText('10240.00 MB')).toBeInTheDocument();
    expect(screen.getByText('92160.00 MB')).toBeInTheDocument();
  });
});
